# 🧗‍♂️ Indoor Climbing Hold Segmentation (Detectron2)

This project builds an end-to-end **computer vision pipeline** that segments **holds**, **volumes**, and **downclimb markers** on indoor climbing walls using **Mask R-CNN** (via Facebook AI’s [Detectron2](https://github.com/facebookresearch/detectron2)).

It combines **dataset handling**, **COCO annotation registration**, **model training/evaluation**, and **visualization** into one workflow — enabling automatic route understanding in climbing gyms.
...
| Path | Description |
|------|--------------|
| `src/hold_detectron.py` | Reproducible CLI script — trains, evaluates, and saves sample visualizations. |
| `notebooks/hold_detectron2_notebook.ipynb` | Readable Colab-style notebook — step-by-step exploration of the same pipeline. |
...
The project expects **three datasets** (Task A, B, C) in **COCO format** — each containing an `instances_default.json` file and its corresponding `images/` folder.
...
> If you’d like to reproduce the experiment, please **contact me** to request access.
...
> 💡 **Windows users:** Detectron2 doesn’t provide official Windows wheels.  
...
| **`.py`** | Command-line reproducibility — clean, parameterized training & evaluation. |
...
- Introduce **hold-type classification** — train a secondary model to recognize specific hold shapes such as *jugs, crimps, pockets, slopers,* and *pinches.*  
